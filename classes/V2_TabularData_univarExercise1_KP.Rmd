...
# prop.table always means table data as proportions of overall 
```
...
# aes always refers to the X and Y of the plot
```
...
# as.numeric turns levels of the factor into numbers, because as grades they are simply categorical
...
frequency(eduwa3$redLunchOrd,style = 'rmarkdown',report.nas = F)
```
...
```{r baseHistogramFree, eval=FALSE}
...
eduwa3$redLunchOrd=cut(eduwa3$Free.Lunch,
                       breaks = dataHist$breaks,
                       include.lowest = T,
                       ordered_result = T)
```
...
```{r TitleFreeLunch, eval=FALSE}
titleText4="Rates at Which Schools Offer Free Lunch to Students"
```

```{r histFreeLunch, eval=FALSE}
hist(eduwa3$Free.Lunch,
     labels = T,
     xlab="Schools Offering Lunches", 
     cex.lab = 0.8,
     xaxt="n",
     ylim=c(0,1000),
     main=titleText4)
...
```{r tapply,  eval=FALSE}
# tapply(variable = eduwa$student.teacher.ratio,group = the county,functionToApply = mean)
...
Because measurements are continuous values, a **density** plot is more appealing to their nature:
...
```{r ggplot_Histogram, eval=FALSE}
base= ggplot(data = eduwa5, aes(x= Student.Teacher.Ratio), na.rm=TRUE) +
        geom_histogram(breaks=seq(5, 30, by=2),
                       ...
                       na.rm = TRUE,
                       inherit.aes = TRUE) + 
  labs(title="WA Student Teacher Ratios", subtitle =sourceText, x="Student Teacher Ratio", y="Number of Schools") + 
  xlim(c(5,30))+
  ylim(c(0,600))
...
```{r}
tableFreq=as.data.frame(eduwa5$Student.Teacher.Ratio)
tableFreq
...
meanValues2=tapply(eduwa5$Student.Teacher.Ratio, 
                  ...
                  mean)
hist(meanValues2)
abline(v=mean(meanValues2),lty=3,lwd=3,col='blue')
...
density1= base + geom_vline(aes(xintercept=mean(Student.Teacher.Ratio)),
            color="blue", linetype="dashed", size=1) + geom_vline(aes(xintercept=median(Student.Teacher.Ratio)),
